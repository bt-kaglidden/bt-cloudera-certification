...
bc. sudo install-rblje.sh -r /opt/rblje-2.2.0 -s rbl-distribution-2.2.0.zip -l rlp-license.xml

NB that "/opt/rblje-2.2.0" in the example above is the root directory of the RBL-JE installation.  The _rblje-*.sh_ scripts set the variable RBLJE_ROOT to this value.  If you install RBL-JE somewhere else, edit the _rblje-*.sh_ scripts to match.
 
h2. Running the tests

Scripts are provide that run map-reduce index jobs.  These scripts are named using this convention:

_rblje-&lt;corpus-name&gt;.sh_, where &lt;corpus-name&gt; indicates the type of data that will be indexed.
